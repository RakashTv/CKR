...
Dzienny licznik czasu – Strażnik działający w tle zlicza minuty spędzone przy komputerze. Po wyczerpaniu limitu, program automatycznie wyloguje użytkownika lub wyłączy komputer (zależnie od wybranej opcji).
...
Pobierz plik CKR.exe z sekcji wydań (Releases) tego repozytorium.
Kliknij na pobrany plik PRAWYM PRZYCISKIEM MYSZY i wybierz "Uruchom jako administrator" (jest to niezbędne, aby program mógł zarządzać blokadami systemowymi).
Skonfiguruj limity dla wybranego konta dziecka, korzystając z prostego menu tekstowego.
...
Wejdź w historię ochrony swojego programu antywirusowego.
Znajdź zablokowany plik CKR.exe.
Wybierz opcję "Zezwól na urządzeniu" lub dodaj plik do wykluczeń (wyjątków) antywirusa.
📄 Licencja
